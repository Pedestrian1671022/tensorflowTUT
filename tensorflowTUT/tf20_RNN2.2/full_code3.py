...
def BiRNN(x, weights, biases):
    ...
    # state_fw = lstm_fw_cell.zero_state(batch_size, dtype=tf.float32)
    # lstm_bw_cell = tf.contrib.rnn.BasicLSTMCell(n_hidden, forget_bias=1.0)
    # state_bw = lstm_bw_cell.zero_state(batch_size, dtype=tf.float32)
    ...
    (outputs, states) = tf.nn.bidirectional_dynamic_rnn(lstm_fw_cell, lstm_bw_cell, x, initial_state_fw=state_fw,
                                                               initial_state_bw=state_bw)
    print(outputs, states)
    _inputs = tf.concat([states[0].h, states[1].h], 1)
