...
with tf.name_scope("conv"):
    with tf.name_scope("conv1"):
        W_con1 = tf.Variable(tf.truncated_normal([3, 3, 3, 32], stddev=0.1), name="W_con1")
        b_con1 = tf.Variable(tf.constant(0.1, shape=[32]), name="b_con1")
    ...
    with tf.name_scope("conv2"):
        W_con2 = tf.Variable(tf.truncated_normal([3, 3, 32, 64], stddev=0.1), name="W_conv2")
        b_con2 = tf.Variable(tf.constant(0.1, shape=[64]), name="b_conv2")
    ...
    with tf.name_scope("conv3"):
        W_con3 = tf.Variable(tf.truncated_normal([3, 3, 64, 128], stddev=0.1), name="W_conv3")
        b_con3 = tf.Variable(tf.constant(0.1, shape=[128]), name="b_conv3")
    ...
    with tf.name_scope("conv4"):
        W_con4 = tf.Variable(tf.truncated_normal([3, 3, 128, 256], stddev=0.1), name="W_conv4")
        b_con4 = tf.Variable(tf.constant(0.1, shape=[256]), name="b_conv4")
...
with tf.name_scope("full_connection_and_output"):
    with tf.name_scope("full_connection"):
        W_fc1 = tf.Variable(tf.truncated_normal([14*14*256, 1024], stddev=0.1), name="W_fc1")
...
with tf.name_scope("loss"):
    ...
    loss = tf.losses.softmax_cross_entropy(onehot_labels=one_hot_labels, logits=y_conv)

with tf.name_scope("train"):
    global_step = tf.Variable(initial_value=0, dtype=tf.int32, name='global_step', trainable=False)
    lr = tf.train.exponential_decay(learning_rate=1e-4, global_step=global_step, decay_steps=int(train_size*2/batch_size), decay_rate=0.8,
                                    staircase=True)
...
# saver.export_meta_graph("ckpt/model.ckpt.meta.json", as_text=True)
...
with tf.Session() as sess:
    ...
    for epoch in range(epochs):
        for step in range(int(train_size/batch_size)):
            ...
            _loss, _accuracy, _ = sess.run([loss, accuracy, train_step], feed_dict={x_: img_train, y_: label_train})
            if step % 10 == 0:
